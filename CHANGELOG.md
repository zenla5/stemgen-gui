...
- **[CI-SECURITY-AUDIT]** Remaining open vulnerabilities are all out-of-scope of #7 (wdio/`deepmerge-ts`, `serialize-javascript`, `extract-zip`, tsx-bundled esbuild) and are tracked as follow-ups; the `npm audit` gate stays `|| true` server-side.
