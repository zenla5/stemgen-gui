...
- **Version consistency** — All version strings bumped to 1.5.9: `package.json`, `Cargo.toml` (workspace), `src-tauri/Cargo.toml`, `src/lib/constants.ts` (`APP_VERSION`), and `src-tauri/tauri.conf.json`.
...
- **[MODEL-VERSION]** The AI Models panel now shows the installed revision + date for each downloaded model (e.g. `rev cbc8a9b1 · 2026-09-02`), resolved from the HuggingFace cache via `scan_cache_dir()`; direct `.onnx` downloads get a sha256/date checksum sidecar. A new `get_model_statuses` backend command merges the sidecar's HF-cache status with the app models dir and is the single availability source for the panel (Refs #265).
- **[MODEL-UPDATE-INDICATOR]** The AI Models panel reports "Update available" only when the sha256 of a file demucs actually loads (`htdemucs.yaml` + `<sig>.safetensors`) differs from upstream `main`, with a working re-download (`snapshot_download(force_download=True)`) that re-fetches changed files. Otherwise it shows "Up to date". Upstream last-modified is displayed as informational text only and never triggers an update by itself. The check runs on-demand via the existing Refresh button and fails gracefully when offline (Refs #265).
...
- **Version consistency** — All version strings bumped to 1.5.8: `package.json`, `Cargo.toml` (workspace), `src-tauri/Cargo.toml`, `src/lib/constants.ts` (`APP_VERSION`), and `src-tauri/tauri.conf.json`.
...
- **Version consistency** — All version strings bumped to 1.5.5: `package.json`, `Cargo.toml` (workspace), `src-tauri/Cargo.toml`, `src/lib/constants.ts` (`APP_VERSION`), and `src-tauri/tauri.conf.json`.
...
- **Version consistency** — All version strings bumped to 1.5.4: `package.json`, `Cargo.toml` (workspace), `src-tauri/Cargo.toml`, `src/lib/constants.ts` (`APP_VERSION`), and `src-tauri/tauri.conf.json`.
