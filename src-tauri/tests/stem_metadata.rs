...
use stemgen_gui_lib::stems::metadata::{MasterData, NIStemMetadata, StemData, StemType};
...
fn test_metadata_default_produces_valid_json() {
    ...
    let deserialized: NIStemMetadata =
        NIStemMetadata::from_json_bytes(&json_bytes).expect("metadata must be JSON deserializable");
