...
fn test_waveform_different_sample_rates() {
    ...
    assert!(
        (wf_44100.duration_secs - 1.0).abs() < 0.1,
        "44100 Hz waveform should be ~1s, got {}",
        wf_44100.duration_secs
    );
    assert!(
        (wf_22050.duration_secs - 1.0).abs() < 0.1,
        "22050 Hz waveform should be ~1s, got {}",
        wf_22050.duration_secs
    );
