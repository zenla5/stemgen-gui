...
use stemgen_gui_lib::stems::metadata::{NIStemMetadata, StemData, MasterData};
...
fn make_mp4_with_nmde() -> Vec<u8> {
    ...
    nmde.write_all(&u32::to_be_bytes(nmde_total as u32)).unwrap();
    ...
    let mut udta = Vec::new();
    udta.write_all(&u32::to_be_bytes((8 + nmde.len()) as u32)).unwrap();
    udta.write_all(b"udta").unwrap();
    udta.extend_from_slice(&nmde);

    let mut moov = Vec::new();
    moov.write_all(&u32::to_be_bytes((8 + udta.len()) as u32)).unwrap();
    moov.write_all(b"moov").unwrap();
    moov.extend_from_slice(&udta);
...
fn test_nmde_atom_injected_into_empty_moov() {
    ...
    assert!(result.is_ok(), "embed_metadata_atom should succeed: {:?}", result);

    let buf = std::fs::read(&mp4_path).unwrap();

    // Verify nmde atom exists
    let (nmde_start, nmde_end) = find_atom(&buf, b"nmde")
        .expect("nmde atom must be present after injection");
    assert!(nmde_end > nmde_start + 8);

    // Verify "stem" marker is in nmde payload
    let payload = &buf[nmde_start + 8..];
    assert!(payload.starts_with(b"stem"), "nmde payload should start with 'stem' marker");
    
    // Verify JSON follows
    assert!(payload[5..].starts_with(b"{\"version\""), "nmde should contain JSON");

    // Verify udta wraps nmde
    let (udta_start, udta_end) = find_atom(&buf, b"udta")
        .expect("udta atom must be present");
    assert!(nmde_start >= udta_start && nmde_end <= udta_end, "nmde should be inside udta");
...
fn test_nmde_replaces_existing_nmde_in_udta() {
    ...
    assert_eq!(nmde_positions.len(), 1, "should have exactly one nmde atom after replacement");
    ...
    assert!(json_str.contains("version"), "updated nmde should contain JSON");
    assert!(!json_str.contains("test"), "old data 'test' should be replaced by JSON");
...
fn test_metadata_sidecar_written_when_no_moov() {
    ...
    assert!(sidecar_path.exists(), "sidecar metadata file should be created as fallback");
    let sidecar_content = std::fs::read_to_string(&sidecar_path).unwrap();
    assert!(sidecar_content.contains("version"), "sidecar should contain metadata JSON");
...
fn test_nmde_atom_roundtrip_survives_multiple_injections() {
    ...
    assert!(find_atom(&buf1, b"nmde").is_some(), "nmde should exist after first injection");

    // Second injection (should replace the first)
    let metadata2 = {
        let stems = vec![StemData { name: "Vocals".to_string(), color: "#95E1D3".to_string(), file_path: "vocals.m4a".to_string() }];
        let master = MasterData { name: "Master2".to_string(), file_path: "master2.m4a".to_string() };
        NIStemMetadata::new(stems, master)
    };
    ...
    let nmde_count = {
        ...
        while offset + 8 <= buf2.len() {
            let size = u32::from_be_bytes([buf2[offset], buf2[offset + 1], buf2[offset + 2], buf2[offset + 3]]) as usize;
            if &buf2[offset + 4..offset + 8] == b"nmde" { count += 1; }
            if size < 8 { break; }
            offset += size;
        }
        count
    };
    assert_eq!(nmde_count, 1, "exactly one nmde atom after second injection");

    // JSON should be the second one
    let (nmde_start, _) = find_atom(&buf2, b"nmde").unwrap();
    let payload = &buf2[nmde_start + 8..];
    assert!(payload[5..].contains("Master2"), "second metadata should be in nmde after replacement");
