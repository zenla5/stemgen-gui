    ...
    pub fn from_samples(samples: &SampleData, points_per_second: u32) -> Self {
        ...
        for i in 0..total_points {
            ...
            let segment_samples: Vec<f32> = (start..end)
                .map(|frame| {
                    // Mix to mono
                    samples.samples[frame * samples.channels as usize..]
                        [..samples.channels as usize]
                        .iter()
                        .sum::<f32>()
                        / samples.channels as f32
                })
                .collect();
            ...
            let min = segment_samples
                .iter()
                .cloned()
                .fold(f32::INFINITY, f32::min);
            let max = segment_samples
                .iter()
                .cloned()
                .fold(f32::NEG_INFINITY, f32::max);

            let sum_squares: f32 = segment_samples.iter().map(|s| s * s).sum();
